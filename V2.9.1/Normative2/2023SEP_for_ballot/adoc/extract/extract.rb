...
module V2AD
  ...
  def todo
    todos = [
      "Each event extraction method needs to set event.responses and/or event.ack",
      "Go back to 11.4.1 and finish it."
    ]
    puts Rainbow("TODO:").lime
    todos.each { |td| puts " * #{td}" }
    puts
  end
  ...
  def extract_ch(ch)
    ...
    files.each do |adoc_file|
      ...
      unless section =~ /\.0$/
        raise "No section obj for #{section.num}" unless section_obj
      end
    end
    ...
    files.each do |adoc_file|
      ...
      #   puts Rainbow("No section_obj for #{section.num}").orange
      # end
      
      # section_obj = V2AD.v2.sections.dig(section, :obj)
      # if section =~ /\.0$/ && section_obj
      #   puts Rainbow("Zero section_obj for #{section.num}").orange
      ...
      unless section =~ /^2A?/
        is_event           = false
        has_message_struct = section_tables.find { |table| table.type.to_s =~ /message_structure/ }
        ...
        if found1 = lines.find { |str| str =~ /==+.*\(\s*[E|e]vent/ }
          ...
          unless section =~ /5\.9\.(1|3|4|5|6)/ || section == '13.3.6.1'
            puts Rainbow("Response Event in #{section.num}").lime if verbose || opts[:verbose]
            is_event = true
            opts[:response] = true
          end
        ...
        if is_event && has_message_struct
          ...
          extract_events(section_obj, opts)
        end
        ...
        if is_event && !has_message_struct
          ...
          if lines.find { |l| l =~ /withdrawn|Retained for backwards compatibility/i }
            extract_withdrawn_events(section_obj, opts)
          elsif lines.first =~ /4\.4\.(9|11|13)\s/
            # message structs are in subsections...
          elsif lines.first =~ /10\.3\.\d+\s/
            # puts section
            # puts lines
            extract_event_ch10(section_obj, opts)
          elsif lines.first =~ /10\.4\.\d+\s/
            extract_event_ch10(section_obj)
          else
            if section =~ /11\.4\.(2|3|4|5)/
              extract_11_4(section_obj)
            elsif section =~ /11\.5\.(2|3|4|5)/
              extract_11_5(section_obj, opts)
            elsif section =~ /11\.6\.(3|4|5)/
              extract_11_6(section_obj, opts)
            elsif section =~ /16\.3\.1(5|6)/
              extract_undefined_event(section_obj, opts)
            else
              unless section == '4.4.7' || section =~ /5\.9\.(1|3|4|5|6)/
                section_obj.tables.each(&:display)
                puts Rainbow("Alert! No msg struct! #{section.inspect}").red + ' ' + lines.first
              end
            end
        ...
        if has_message_struct && !is_event
          if section =~ /5\.3\.\d\.\d/ || section == '5.7.3.1' || section =~ /^5\.9\.\d\.\d\.\d/ || section == '5.9.2.4' || section == '5.9.5.1' || section == '5.9.6.1' || section == '5.9.7.1' || section == '5.9.7.2' || section == '4.6.2' || section == '4A.3.23'
            ...
            puts Rainbow("Section #{section.num} has message structure with no event.").red unless section == '10.3' || section == '10.4' || section == '11.5.1'
          end
        end
      
        if section == '10.3'
          extract_10_3(section_obj, opts)
        end
            
        if section == '10.4'
          extract_10_4(section_obj, opts)
        end
      
        if section == '11.5.1'
          extract_11_5_1(section_obj, opts)
        end
      end
...
# next
      ...
      if is_segment && !has_seg_def
        ...
        puts Rainbow("Alert! No seg def! #{section.num}").red# + ' ' + lines.first
        
        lines.each { |l| puts Rainbow(l).lightskyblue }
      end
      if is_segment && has_seg_def
        extract_segment(section_obj, opts)
      end
